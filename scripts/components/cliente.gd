...
func sentarse(asiento_obj: Variant):
	estado = "sentado"
	if asiento_obj is Dictionary:
		global_position = asiento_obj.get("posicion", global_position)
		global_rotation = asiento_obj.get("rotacion", global_rotation)
	elif asiento_obj is Node3D:
		asiento = asiento_obj
		var pose: Dictionary = UtilAsientoScript.pose_desde_nodo(asiento_obj)
		global_position = pose.get("posicion", asiento_obj.global_position)
		global_rotation = pose.get("rotacion", asiento_obj.global_rotation)
